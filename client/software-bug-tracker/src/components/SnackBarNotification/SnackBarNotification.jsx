...
const SnackBarNotification = ({ connectedUser, disconnectedUser, show, onShow, onClose, duration = 7000 }) => {
    ...
    return (
            <div id="snack-bar-notification-container" className={`${show ? "show" : ""} atma-light`}>
                ...
                <h1 id="snack-bar-notification-header">{connectedUser && connectedUser.name ? `${connectedUser.name} has logged into Live Support` : disconnectedUser && disconnectedUser.name ? `${disconnectedUser.name} has logged out of Live Support` : ""}</h1>
            </div>
    )
}
